...
namespace RockWeb.Blocks.CheckIn
{
    ...
    public partial class Search : CheckInBlock
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (searchTypeValue == Rock.SystemGuid.DefinedValue.CHECKIN_SEARCH_TYPE_PHONE_NUMBER.AsGuid())
                {
                    ...
                    lblSearchTitle.Text = "Search By Phone";
                }
                else
                {
                    pnlSearchName.Visible = true;
                    pnlSearchPhone.Visible = false;
                    lblSearchTitle.Text = "Search By Name";
                }
            }
        }
        ...
        {
            var searchTypeValue = GetAttributeValue("SearchType").AsGuid();
            var searchTypeName = "phone number";
            if (searchTypeValue != Rock.SystemGuid.DefinedValue.CHECKIN_SEARCH_TYPE_PHONE_NUMBER.AsGuid())
            {
                searchTypeName = "name";
            }

            ProcessSelection(maWarning, () => CurrentCheckInState.CheckIn.Families.Count <= 0, "<p>There are not any families with the selected " + searchTypeName + "</p>");
        }
